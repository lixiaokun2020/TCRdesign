...
def main(config):
    ...
    BERT_params = list(map(id, model.BetaModel.parameters())) +  list(map(id, model.AlphaModel.parameters()))  +list( map(id, model.AntigenModel.parameters()))
    ...
    logger.info(f'Saving three berts to {beta_bert_save_dir}, {alpha_bert_save_dir} and {antigen_bert_save_dir}')
    os.makedirs(beta_bert_save_dir)
    model.BetaModel.save_pretrained(beta_bert_save_dir)
    os.makedirs(alpha_bert_save_dir)
    ...
    test_output = trainer.test(tcr_tokenizer=tcr_tokenizer,
